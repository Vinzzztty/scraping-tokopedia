...
async function getProducts(keyword, filename) {
    ...
    const products = await page.evaluate(() => {
        ...
        const productElements = document.querySelectorAll(
            ".prd_link-product-name.css-3um8ox"
        );
        ...
        const ratingElements = document.querySelectorAll(
            "div.prd_shop-rating-average-and-label > span.prd_rating-average-text"
        );
        const productRatings = Array.from(ratingElements).map((rating) =>
            rating.innerText.trim()
        );

        const locationElements = document.querySelectorAll(
            "div.css-1rn0irl > span.prd_link-shop-loc"
        );
        const productLocations = Array.from(locationElements).map((location) =>
        ...
        const productShops = Array.from(shopElements).map((shop) =>
            shop.innerText.trim()
        );

        const soldElements = document.querySelectorAll(
            "span.prd_label-integrity"
        );
        const productSolds = Array.from(soldElements).map((sold) =>
            sold.innerText.trim()
        );

        const discountElements = document.querySelectorAll(
            ".prd_badge-product-discount"
        );
        const productDiscounts = Array.from(discountElements).map((discount) =>
            discount.innerText.trim()
        );

        const images = Array.from(
            document.querySelectorAll(".pcv3_img_container > img")
        );
        const productImages = images.map((img) => img.src);

        for (let i = 0; i < Math.min(productNames.length); i++) {
            const discount =
                productDiscounts[i] !== undefined ? productDiscounts[i] : 0;
